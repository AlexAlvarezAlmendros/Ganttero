import { z } from "zod";
import { type CapturedItem, capturedItemSchema } from "./schema.js";

/**
 * Transcripción → JSON de tarea con Gemma 4 vía Ollama.
 *
 * Doble cinturón: el JSON schema en `format` restringe la generación
 * (structured output de Ollama) y Zod valida el resultado; si Zod falla,
 * se reintenta pasando el error como feedback.
 */

/**
 * Espejo JSON Schema de `capturedItemSchema` para el `format` de Ollama.
 * Solo tipos y enum: las restricciones finas (pattern de fecha, min/max…)
 * rompen el compilador de gramática de Ollama ("failed to parse grammar",
 * visto con 0.20.2). De ellas se encarga Zod + reintento.
 */
...
export interface StructureOptions {
	/** Fecha de referencia YYYY-MM-DD para resolver fechas relativas. Siempre inyectada. */
	today: string;
	model?: string;
	baseUrl?: string;
	maxAttempts?: number;
...
export async function structureTranscript(
	...
	for (let attempt = 1; attempt <= maxAttempts; attempt++) {
		...
		lastRaw = payload.message.content;

		let candidate: unknown;
		try {
			candidate = JSON.parse(lastRaw);
		} catch {
			candidate = null; // JSON roto → reintento con feedback
		}
