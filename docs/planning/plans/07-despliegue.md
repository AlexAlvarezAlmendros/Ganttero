...
| Fecha | Tarea | Notas |
|-------|-------|-------|
| 2026-07-24 | 7.1 (fix) | Primer `docker compose build` real en el homeserver: el build del backend fallaba con `Unknown option: 'legacy'`. Causa: el Dockerfile usaba `pnpm deploy --prod --legacy /out` (la opción `--legacy` es de pnpm 10) pero el repo fija `pnpm@9.15.9` vía corepack. Corregido quitando `--legacy`. Imagen construida y app verificada corriendo (`/api/health` → `{"status":"ok","db":"ok"}`). |
